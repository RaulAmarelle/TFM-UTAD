...
```{r}
# Cambiamos la clase a FECHA, para que pase de chr a Date
...
```{r}
## Puede ser interesante crear una columna categorica Entre semana (ES) y Fin de semana (FS), para analizar posibles cambios de comportamiento
...
Si nos fijamos en la linea roja, se aprecia que hay una ligera tendencia de disminucion del consumo a lo largo de estos dos anios, motivada por la bajada de consumo de los ultimos 3-4 meses. 
Para extraer alguna conclusion mas solida habria que analizar un periodo mas amplio, pues al ser la disminucion más pronunciada al final, podria ser por un motivo coyuntural en una epoca del año con extraordinario buen tiempo.
...
Tambien podemos ver que más del 50% de los dias el sistema de climatizacion funciona en regimen de calefaccion y poco mas de un tercio en refrigeracion
...
ggplot(df.data, aes(WEEKDAY, AFLUENCIA)) + geom_boxplot() +
  labs(x = "Dia de la semana",y = "Personas") + 
  ggtitle ('Afluencia segun dia de la semana') + 
  theme(plot.title = element_text(hjust = 0.5))
```

Sorprendentemente, los dias de mayor asistencia son miercoles y jueves. Esperaba que fueran los dias del fin de semana
...
ggplot(df.data, aes(MONTH, AFLUENCIA)) + geom_boxplot() +
...
Tambien en mi opinion de manera sorprendente, los meses de mayor afluencia son julio y agosto, meses que supones que la gente esta de vacaciones y sale mas con el buen tiempo
...
ggplot(df.data, aes(WEEKDAY, KWH)) + geom_boxplot() +
  labs(x = "Dia de la semana",y = "KWH") + 
  ggtitle ('Consumo Potencia segun dia de la semana') + 
  theme(plot.title = element_text(hjust = 0.5))
```

El dia de la semana que mas consumo hay es el viernes, cuando el dia con mayor asistencia es el miercoles. Tambien vemos que hay un aumento progresivo de desde el lunes (min) a viernes (max).

```{r}
# Analizamos la potencia consumida por meses del año

ggplot(df.data, aes(MONTH, KWH)) + geom_boxplot() +
  labs(x = "Mes",y = "KWH") + 
...
Aunque hay bastante dispersion en los datos, veo una fuerte dependencia entre Consumo vs Afluencia.
Tambien llama la atencion la relacion entre Consumo y CCDD20 (Refrigeracion). Esto es coherente, cuanta mayor diferencia de temperatura con el exterior, mayor consumo. Ya vimos anteriormente que en los meses de verano hay mucho más consumo.
En cambio, en modo Calefaccion (CHDD18), al ser una linea plana significa que para el mismo consumo hay dias de poca diferencia de temperatura y otros dias de mayor gradiente termico.
...
Recopilemos. Tenemos un dataset de un centro comercial. Hemos hecho tratamiento de datos y hemos generado plots para extraer información relevante de las variables.
...
Como podemos ver en el plot anterior, la evolucion de la variable ERROR no es estacional. Como primera aproximacion, podemos observar que el modelo de prediccion ha mejorado a partir de la segunda mitad de anio, ya que el % Error es mucho menor, tiende a concentrarse en la horquilla de -10% a +10%.

Vamos a suponer que a partir de la medicion 426, coincidiendo con el 01/05/2017, se han introducido mejoras en el algoritmo de prediccion y vamos a comparar la mejora
...
El resultado del analisis es que la intervencion redujo la prevision de ERROR en un 87%.


Ahora vamos a probar personalizando el modelo con bsts.model
...
SEASONAL: representa la contribución de cada mes al ciclo anual. Hay que especificarle el número de periodos. Una buena opción puede ser los meses del año.  
...
Opción 1. Tantos períodos como semanas del año
```{r}
starting.point <- 100
end.point <- 465
test.length <- 50
...
Ploteamos a ver que tal las predicciones
```{r}
...
Opción 2. Tantos periodos como meses del año y añadimos MonthlyAnnualCycle
...
Al añadir una nueva componente de tendencia se mejora ligeramente el MAPE. No obstante, las predicciones siguen sin ser muy ajustadas.
...
beta: Si se establece en FALSE, la función hará suavizado exponencial. Es lo que queremos.  
...
Calculamos la previsión para los proximos 6 meses con un intervalo de confianza de 0,95 y representamos el pronóstico junto con los valores reales y ajustados.
...
He estimado las previsiones a 6 meses vista porque la serie es estacionaria y cada 6 meses hay un pico o un valle.
...
```{r}
# Calculo prediccion para los ultimos 2 mesess, a partir 01/01/18
soporte <- window(ts(deseasonal_KWH), start=670)

fit_sin_soporte = arima(ts(deseasonal_KWH[-c(670:725)]), order=c(5,1,7))

prediccion_sin_soporte <- forecast(fit_sin_soporte,h=55)
...
Los resultados de la prediccion no son muy buenos.
Asi que vamos a rehacer el análisis ARIMA, pero descomponiendo la serie con STR.
...
Para la prediccion de la serie temporal, no parece que cobren especial relevancia las variables Potencia Estimada (LB), Dia (DAY) y Anio (YEAR).
Es por esto que vamos a eliminar esas columnas de nuestro dataset de datos y crear el dataframe data.rpart
...
Vemos la importancia que cobra cada variable en la construcción de los árboles de predicción y su peso en la estimación de la potencia consumida
```{r}
tree1$variable.importance
normalize(tree1$variable.importance)
```

Representamos el árbol construido, es decir, tree1
```{r}
...
plot(-log10(prueba.cp), (mape.pruebas.cp), main = 'Métrica MAPE (%) en función del parámetro cp', 
     xlab = 'cp (Transformado como el menos logaritmo decimal: -log10(cp))', ylab = 'Métrica MAPE (%)')
```
...
Plot de la predicción vs el consumo real
```{r}
ggplot() +
...
Se obtiene la misma configuración que en el caso anterio que en el nuevo aunque disminuye ligeramente el MAPE.

Vamos a centrar la búsqueda de los hiperparámetros como se indica:

- Por lo visto en el caso de tree1 y tree2, cuando entrenabamos y predecíamos con la misma serie de datos, a mayor profundiad de los árboles se tiende al overfitting. En estos dos últimos casos lo hemos comprobado viendo que la mejor predicción (menor MAPE), tomaba como max_depth el valor mínimo posible (5). Es por esto que vamos a dirigir la búsqueda de este hiperparámetro hacia valores menores de máxima profundiad.

- En cuanto al mínimo número de splits, y tal como se ha experimentado podemos ver que es 6 para ambos casos y que a su vez es el mayor numero de la serie propuesta. Aumentaremos los valores de la serie

- El valor de cp, de igual manera a la máxima profundidad, cuando entrenábamos y predecíamos con los mismos datos proporcionaba mejores resultados a medida que disminuíamos el parámetro (induciendo al overfitting). En los nuevas predicciones, se ha encontrado como valor el 0.001 (mínimo local) y es en sus valores adyacentes donde centraremos la búsqueda
```{r}
...
paste('cp: ', hp.3.cp)
paste('Mape asociado a esta configuración de hiperparámetros: ', round(mape.3*100,2), '(%)')
...
paste('cp: ', hp.4.cp)
paste('Mape asociado a esta configuración de hiperparámetros: ', round(mape.4*100,2), '(%)')

```


Entrenamos un árbol para las dos series con los óptimos de hiperparámetros encontrados, predecimos y ploteamos:
...
De los 4 modelos analizados, podemos llegar a la conclusion que el modelo con el que hemos obtenido mejor resultado es el ultimo, el RPART.

El ultimo paso es comparar el error existente segun su modelo predictivo y el error calculado segun nuestro nuevo modelo, para decidir cual de los dos modelos predice mejor.
...
Ploteamos para compararlos mejor:
```{r}
...
Segun podemos observar, el nuevo modelo predictivo predice mejor porque la grafica apenas se desvia del 0, mientras el modelo predictivo inicial tiene algunos picos del 15% o del 8% de error.
