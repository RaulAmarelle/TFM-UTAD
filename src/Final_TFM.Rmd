...
```{r}
g <- ggplot(df.data, aes(x=FECHA, y=KWH)) +  geom_line() + geom_smooth(method="lm", colour="red") + geom_smooth(method="loess") + scale_y_continuous(limits = c(0, 20000),"Potencia KWH")
plot(g)
...
ggplot(data = df.data, aes(factor(1), fill=factor(CLIMATIZACION))) + geom_bar(width=1)+ coord_polar(theta="y")
```
...
El dia de la semana que mas consumo hay es el viernes, cuando el dia con mayor asistencia es el miercoles
