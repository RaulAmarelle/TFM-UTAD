...
La dificultad de analizar esta serie viene dada porque esta compuesta por muestras diarias en un ciclo estacional de dos años. Al tener justo el ciclo de dos años, se observa claramente una estacionalidad anual. Pero por otro lado, tampoco es una serie de amplio rango lo suficientemente larga (ej. diez años) ni tampoco en periodos mas cortos se observa una estacionalidad aparente (semana, mes).
...
Como podemos ver en el plot anterior, la evolucion de la variable ERROR no es estacional. Como primera aproximacion, podemos observar que el modelo de prediccion ha mejorado a partir de la segunda mitad de 2017, ya que el % Error es mucho menor, tiende a concentrarse en la horquilla de -10% a +10%.

Vamos a suponer que a partir de la medicion 426, coincidiendo con el 01/05/2017, se han introducido mejoras en el algoritmo de prediccion y vamos a comparar la mejora aplicando Causal Impact.
...
Al añadir una nueva componente de tendencia se mejora ligeramente el MAPE. No obstante, las predicciones siguen lejos del real.
...
El modelo predictivo parece bastante bueno, aunque ofrece dudas por el intervalo de confianza.
...
```{r}
fcast <- forecast(fit2, h=60)  #h=60, a dos meses vista
plot(fcast)
```
...
```{r}
# Calculo prediccion para los ultimos 2 meses, a partir 01/01/18
...
Los resultados predictivos son decepcionantes a corto plazo. He realizado pruebas de simulación a mayor plazo (6 meses, h=180) pero tampoco parece reaccionar la curva. 
